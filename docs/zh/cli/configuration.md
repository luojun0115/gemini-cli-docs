...
- **`GEMINI_CODE_ASSIST`**：
  - 启用 Code Assist 功能。
  - 接受 `true`、`false` 或自定义命令字符串。
  - 如果您使用企业账户，您还应该设置 `GOOGLE_CLOUD_PROJECT` 环境变量。
  - 示例：`export GEMINI_CODE_ASSIST=true`。
- **`GEMINI_SANDBOX`**：
...
- **目的：** 这些 Markdown 文件包含您希望 Gemini 模型在互动期间了解的指令、指南或上下文。系统设计为分层管理此指令上下文。
...
通过理解和利用这些配置层和上下文文件的分层性质，您可以有效地管理 AI 的记忆，并根据您的特定需求和项目定制 Gemini CLI 的响应。
